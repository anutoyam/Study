...
def update_graph(frame_number):
    ...
    Z = np.reshape(z_values, X.shape)
    surf._facecolors2d = surf._facecolors3d
    surf._edgecolors2d = surf._edgecolors3d
    surf.set_array(Z.ravel())
    surf.set_zlim(np.min(Z), np.max(Z))
    return surf,
...
if __name__ == "__main__":
    ...
    surf = ax.plot_surface(np.array(x_values).reshape(-1, 20), np.array(y_values).reshape(-1, 20), np.array(z_values).reshape(-1, 20), cmap='coolwarm')  # Create a 3D surface plot
    ax.set_xlabel('X Values (DBD 0~1916)')
    ax.set_ylabel('Y Values (DBD 4008~5924)')
    ax.set_zlabel('Z Values (DBD 6012)')
    plt.title('PLC Data 3D Surface Plot')
